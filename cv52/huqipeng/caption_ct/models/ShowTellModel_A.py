...
from .NSA_LSTM_att import Encoder as NSA_LSTMEncoder
class ShowTellModel(nn.Module):
    ...
    def forward(self, fc_feats, att_feats, seq):
        ...
        # 实验一 特征平均作为单张数据进行训练，因为LSTM的输入需要是一个向量，二维数据需要进行处理
        # fc_feats = f_feats.mean(1).squeeze()

        # # 实验二 使用NSA_mean 计算输入向量
        sa_feats, nsa_feats = self.NSA_encoder(fc_feats)

        state = self.init_hidden(batch_size)
        outputs = []
        for i in range(seq.size(1)):
            if i == 0:
                xt = self.img_embed(sa_feats)
                # print(xt.shape)
            else:
                if self.training and i >= 2 and self.ss_prob > 0.0: # otherwiste no need to sample
                    sample_prob = sa_feats.data.new(batch_size).uniform_(0, 1)
                ...
                xt = self.embed(it)
                # print(xt.shape)
            output, state = self.core(xt, nsa_feats, state)
    ...
    def sample(self, fc_feats, att_feats, opt={}):
        # 实验一 # 特征平均作为单张数据进行训练，因为LSTM的输入需要是一个向量，二维数据需要进行处理
        # fc_feats = fc_feats.mean(1).squeeze()
        ...
        for t in range(self.seq_length + 2):
            if t == 0:
                ...
                if t == 1: # input <bos>
                    it = sa_feats.data.new(batch_size).long().zero_()
                # elif t == 2: # 第一个生成的单词如果为终止符则替换为概率第二大的单词，保证会生成报告
                #     sampleLogprobs, it = torch.max(logprobs.data, 1)
                #     it = it.view(-1).long()
                #     for i in range(len(it)):
                #         if it[i].item() == 0:
                #             mindata , _ = torch.min(logprobs.data, 1)
                #             logprobs.data[i, 0] = mindata[i]
                #     sampleLogprobs, it = torch.max(logprobs.data, 1)
                #     it = it.view(-1).long()
                elif sample_max:
                    sampleLogprobs, it = torch.max(logprobs.data, 1)
...
class ShowAttendTellCore(nn.Module):
    ...
    def forward(self, xt, att_feats, state):
        att_size = att_feats.numel() // att_feats.size(0) // self.att_feat_size  # 权重分布 img_per_person-2
        ...
        input = torch.cat([xt, att_res], 1).unsqueeze(0)
        output, state = self.rnn(input, state)

        return output, state

class ShowAttendTellModel(ShowTellModel):
    def __init__(self, opt):
