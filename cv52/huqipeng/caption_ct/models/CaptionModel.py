...
class CaptionModel(nn.Module):
    def __init__(self, opt):
        ...
        self.embed = nn.Embedding(self.vocab_size + 1, self.input_encoding_size) #加一是因为有一个0
    ...
    def forward(self, fc_feats, att_feats, seq):
        ...
        state = self.init_hidden(sa_feats)
        ...
        for i in range(seq.size(1) - 1):
            ...
            output, state = self.core(xt, sa_feats, nsa_feats, state)
    ...
    def sample(self, fc_feats, att_feats, opt={}):
        ...
        if beam_size > 1:
            return self.sample_beam(sa_feats, nsa_feats, opt)
...
class ShowAttendTellCore(nn.Module):
    ...
    def forward(self, xt, fc_feats, att_feats, state):
        att_size = att_feats.numel() // att_feats.size(0) // self.att_feat_size  # 24-2
