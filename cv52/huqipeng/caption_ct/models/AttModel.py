...
class AttModel(CaptionModel):
    ...
    def _prepare_feature(self, fc_feats, att_feats, att_masks):
        ...
        fc_feats, att_feats = self.NSA_encoder(fc_feats)
...
class ShowAttendTellCore(nn.Module):
    def __init__(self, opt, use_maxout=False):
        ...
        # self.hid_1 = nn.Linear(opt.rnn_size*3, opt.rnn_size*2)
        # self.hid_2 = nn.Linear(opt.rnn_size*2, opt.rnn_size)
        # self.relu = nn.ReLU(inplace=True)

        ##############attm_lstm###############
        # self.attm_lstm = AttmLSTM(opt,opt.rnn_size*3)
        ######################################

    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, att_masks=None):
        att = self.attention(state[0][-1], att_feats, p_att_feats, att_masks)

        lang_lstm_input = torch.cat([xt, att], 1)
        h_lang, c_lang = self.lang_lstm(lang_lstm_input, (state[0][-1], state[1][-1]))

        output = F.dropout(h_lang, self.drop_prob_lm, self.training)
        state = (h_lang.unsqueeze(0),
                 c_lang.unsqueeze(0))
...
class ShowAttendTellModel(AttModel):
    def __init__(self, opt):
