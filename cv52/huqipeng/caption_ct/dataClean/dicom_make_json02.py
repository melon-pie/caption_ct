...
def pre_pro():
    patients = {}
    root_path = r'E:\data\NewCT\正常'
    annotation_xlsx = r'E:\data\NewCT\正常.xlsx'
    json_patients = r'E:\data\data1023\patients02.json'

    # 遍历读取Excel信息录入patients
    patient_num = 0
    excel_data = pd.read_excel(annotation_xlsx, sheet_name='Sheet1', converters={'acc':str})
    # data = np.array(excel_data.loc[2,:])
    # print( data)
    for i in range(len(excel_data['acc'])):
        acc_num = excel_data['acc'][i]
