...
def pre_pro():
    ...
    saved_imgs = 0
    mkdir = 0
    json_patients = r'E:\data json file\patients02.json'
    json_error_patients = r'E:\data json file\patients02_error.json'
    save_root = r'E:\test'
    ...
    for acc, patient in patients.items():
        ...
        if len(list(patient["StudyInstanceID"].keys())) == 0:
            ...
            continue
        study = list(patient["StudyInstanceID"].keys())[-1]
        series_cov_name = patient["StudyInstanceID"][study]["series_id"][-1]
        series = patient["SeriesInstanceUID"][series_cov_name]
        # total_img += series["num"]
        save_num = str(patients[acc]['numkey'])
        save_path = save_root + '\\' + save_num + '_02'
        ...
        for root, dirs, files in os.walk(patient_dir):
            if (len(files) > 0) & (len(dirs) == 0):
                for i, file in enumerate(files):
                    ...
                    # 检查扫描顺序是否是空间位置顺序
                    # 有一个错误文件跳过
                    if file == '1.2.840.113619.2.428.3.403321453.349.1596699726.22':
                        error_patients[acc] = patient
                        error_patients[acc]['error_file_name'] = root + '\\' + file
                        error_img_path = save_path + '\\' + 'IMG' + str('%05d' % i) + '.png'
                        error_patients[acc]['error_img_path'] = error_img_path
                        continue
                    # print('ImagePosition')
                    ...
                    bsb_img = bsb_window(clean_dicom)
                    img_pli = PIL.Image.fromarray(bsb_img.astype(np.int8), mode="RGB")
                    img_save_path = save_path + '\\' + 'IMG' + str('%05d' % i) + '.png'
                    img_pli.save(img_save_path)
