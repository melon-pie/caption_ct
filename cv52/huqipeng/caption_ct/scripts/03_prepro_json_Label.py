...
import jieba
jieba.load_userdict(r"./medical_nouns.txt")
jieba.add_word("破入")
jieba.add_word("邻近")
def prepro_captions(imgs):
    ...
    for i, img in enumerate(imgs):
        ...
        if(type(img['captions'])==str):
            sentence = img["captions"]
            txt = []
            txt.append(" ".join(jieba.cut(sentence, cut_all=False)))
...
def build_vocab(imgs, params):
    ...
    cw = sorted([(count, w) for w, count in sent_lengths.items()], reverse=True)
    ...
    #将小于word_count_threshold 次的单词给换成unk
    for img in imgs:
        img['final_captions'] = []
        for txt in img['processed_tokens']:
            caption = []
...
def assign_splits(imgs, params):
    ...
    val = data.split(' ')
    for i,img in enumerate(imgs):
        id.append(str(img['id']))
        if str(img['id']) in val:
            img['split'] = 'val'
        else:
            img['split'] = 'train'
    json_name = '/home/ai/data/huqipeng/caption_ct/scripts/all_data.json'
...
def main(params):
    ...
    print('label.h5 file is created ')
...
if __name__ == "__main__":
    ...
    # input json
    parser.add_argument('--input_json', default='/home/ai/data/huqipeng/caption_ct/annotations/middle_annotationfile_11_10.json',
                        help='input json file to process into hdf5')
    parser.add_argument('--num_val', default=30000, type=int,
                        help='number of images to assign to validation data (for CV etc)')
    parser.add_argument('--output_json', default='/home/ai/data/huqipeng/caption_ct/data/coco_chinese_talk_11_10.json', help='output json file')
    parser.add_argument('--output_h5', default='/home/ai/data/huqipeng/caption_ct/data/coco_chinese_talk_11_10', help='output h5 file')
    # options
