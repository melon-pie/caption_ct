...
class DataLoader():
    ...
    def get_batch(self, split, batch_size=None, seq_per_img=None):
        ...
        if self.images_per_person == 1:
            ...
            fc_batch = np.ndarray((batch_size, self.images_per_person, self.fc_feat_size), dtype = 'float32')
            att_batch = np.ndarray((batch_size, self.images_per_person, 14, 14, self.att_feat_size), dtype = 'float32')
            # att_batch = np.ndarray((batch_size, 14, 14, self.att_feat_size), dtype = 'float32')
...
class DataLoader_pool(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        # load the json file which contains additional information about the dataset
        print('DataLoader_pool loading json file: ', opt.input_json)
        self.info = json.load(open(self.opt.input_json))
        self.ix_to_word = self.info['ix_to_word']
        self.vocab_size = len(self.ix_to_word)
        print('vocab size is ', self.vocab_size)
        ...
        # open the hdf5 file
        print('DataLoader_pool loading h5 file: ', opt.input_fc_dir, opt.input_att_dir, opt.input_label_h5)
        self.h5_label_file = h5py.File(self.opt.input_label_h5, 'r', driver='core')
